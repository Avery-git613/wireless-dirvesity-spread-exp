...
def selection_combining(received, channel):
    ...
    best_branch = np.argmax(power, axis=0)
    symbol_indices = np.arange(channel.shape[1])
    selected_received = received[best_branch, symbol_indices]
    selected_channel = channel[best_branch, symbol_indices]
...
def simulate_diversity_ber(snr_db_values, num_bits=4000, num_branches=2, seed=2026):
    ...
    for index, snr_db in enumerate(snr_db_values):
        received, channel = rayleigh_fading_branches(symbols, num_branches, snr_db, seed=seed + index)
        single_equalized = received[0] / channel[0]
        sc_equalized = selection_combining(received, channel)
        mrc_equalized = maximal_ratio_combining(received, channel)

        single_ber.append(calculate_ber(bits, bpsk_demodulate(single_equalized)))
        sc_ber.append(calculate_ber(bits, bpsk_demodulate(sc_equalized)))
        mrc_ber.append(calculate_ber(bits, bpsk_demodulate(mrc_equalized)))

    return {'单分支': single_ber, 'SC': sc_ber, 'MRC': mrc_ber}
...
def equal_gain_combining(received, channel):
    ...
    weights = np.conj(channel) / np.abs(channel)
    return np.sum(weights * received, axis=0) / received.shape[0]
